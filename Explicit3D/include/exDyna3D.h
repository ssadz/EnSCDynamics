...
#include <string>       // 包含 std::string (可能被间接引入，但这里明确包含它)
...
namespace EnSC {
	...
	protected:
		...
		void generateVirtualParticlesForFace(int faceIdx, int nLayers, int particlesPerEdge, const std::vector<Types::Real>& unitCoords, std::vector<std::array<Types::Real, 3>>& unitPoints, std::vector<int>& particleIndices, int& pointIdx, int& particleIndex, const Eigen::Matrix<Types::Real, 8, 3>& xyzMatrix, const Eigen::Matrix<Types::Real, 8, 3>& velMatrix); // 为各个面生成虚拟粒子
